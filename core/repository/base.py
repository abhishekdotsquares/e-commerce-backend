...
class BaseRepository(Generic[ModelType]):
    ...
    async def update(self, id, attributes: dict[str, Any] = None) -> ModelType:
        """
        ...
        model = await self._first(select(self.model_class).where(self.model_class.id == id))
        
        if not model:
            raise ValueError(f"Model with ID {attributes['id']} not found.")
